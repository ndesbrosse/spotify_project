...
def render_duel(pair_data, selection_state, records):
    ...
    left_feat = (
        f"Feat : {left['feat']}" if left["feat"] != "None" else "Pas de featuring"
    )
    right_feat = (
...
def make_ranking_set(path, n, records):
    if path != "/ranking" or not records:
        return no_update
    df = pd.DataFrame(records)
    n = min(10, len(df))
    sample = df.sample(n, replace=False).to_dict("records")
    return sample
...
@app.callback(
    Output("rank-results", "children"),
    Input("rank-order", "data"),
    State("ranking-tracks", "data"),
    prevent_initial_call=True,
)
def validate_ranking(order_ids, tracks):
